...
SPECIAL_PAIRS_CONFIG = {
    '1524': 0.70,  # Kare (İkili eşleşme sıkılığı)
    '1560': 0.50   # Dikdörtgen
}

# Yeni Paket (Grup) Profili Ayarları
PACKAGE_PROFILES_CONFIG = {
    '9859': 6      # Profil adı ve bir paketteki yan yana profil sayısı
}
...
class DXFProfileLoader:
    ...
    def load_dxf(self, dxf_path, profile_name):
        ...
        try:
            ...
            for e in msp:
                if e.dxftype() == 'LINE':
                    G.add_edge(to_key((e.dxf.start.x, e.dxf.start.y)), to_key((e.dxf.end.x, e.dxf.end.y)))
                    has_data = True
                    ...
                    has_data = True
                # YENİ: Yay ve Çember Desteği (Test Tuner'dan eklendi)
                elif e.dxftype() == 'ARC':
            ...
            print(f"DXF Yükleme Hatası ({profile_name}): {e}")
            return None
...
class ProfileRenderer:
    ...
    def apply_aluminum_texture(self, mask):
        ...
        if random.random() < 0.5:
            for y in range(h):
                v = int(np.clip(base_val - contrast_val * np.sin(y / h * 3.14), 0, 255))
                img_bgr[y, :, :] = (v, v, v)
        else:
            for x in range(w):
                v = int(np.clip(base_val - contrast_val * np.sin(x / w * 3.14), 0, 255))
                img_bgr[:, x, :] = (v, v, v)
    ...
    # YENİ: Paket Oluşturucu (Test Tuner'dan eklendi)
    ...
    def render_profile_sprite(self, contour, size, rotation=0, profile_name="default"):
        ...
        # Dik yatırma güvencesi (9859 vb. düz profiller için)
        if profile_name in PACKAGE_PROFILES_CONFIG and sprite.shape[1] > sprite.shape[0] and rotation == 0:
...
class SyntheticPalletGenerator:
    ...
    def generate_tunnel_void(self, w, h, obj_cx, obj_cy, thickness, ptype):
        ...
        # Paket profillerin kalınlığı bazen büyük olabilir, bu yüzden minimum bir koruma ekliyoruz.
        void_w = max(w - 2 * thickness, w - 4)
        void_h = max(h - 2 * thickness, h - 4)
        ...
        # Opaklık profiline göre ayarlanır
        if ptype == '1524': base_opacity = random.uniform(0.92, 0.99)
        elif ptype in PACKAGE_PROFILES_CONFIG: base_opacity = random.uniform(0.80, 0.98)
        else: base_opacity = random.uniform(0.80, 0.95)
    ...
    # YENİ: Render Komutlarını Sıraya Alan Yardımcı Fonksiyon
    def _queue_render(self, depth_list, face_list, sprite, x, y, ptype, ptype_idx, thickness):
        ...
        void_sprite = self.generate_tunnel_void(sw, sh, obj_cx, obj_cy, thickness, ptype)
        
        depth_list.append({
            'pos_x': int(x), 'pos_y': int(y), 'sw': int(sw), 'sh': int(sh), 'alpha': sprite[:,:,3],
            'vec_x': vec_x, 'vec_y': vec_y, 'dist': dist, 'depth_len': depth_len, 'void_sprite': void_sprite
        })
        face_list.append({
            'sprite': sprite, 'pos_x': int(x), 'pos_y': int(y), 'sw': int(sw), 'sh': int(sh), 'ptype_idx': ptype_idx
        })
    ...
    def generate_pallet(self, specific_ptype):
        ...
        # DURUM 1: İKİLİ EŞLEŞEN PROFİLLER (Eski Mantık - 1524, 1560)
            ...
            for _ in range(pile_count):
                ...
                for r in range(grid_rows):
                    ...
                    for c in range(grid_cols):
                        ...
                        self._queue_render(render_commands_depth, render_commands_face, sprite, final_x, final_y, current_pile_ptype, ptype_idx, t)
        ...
        # DURUM 2: PAKETLİ PROFİLLER (Test Tuner'dan Eklendi - 9859 vb.)
        elif specific_ptype in PACKAGE_PROFILES_CONFIG:
            profiles_per_pkg = PACKAGE_PROFILES_CONFIG[specific_ptype]
            profile_contour = self.loader.profiles[specific_ptype]
            base_size = int(random.randint(30, 45) * scale_factor)
            
            base_sprite, _, t = self.renderer.render_profile_sprite(profile_contour, base_size, 0, specific_ptype)
            if base_sprite.shape[0] == 0 or base_sprite.shape[1] == 0: 
                return pallet, [], {}

            pkg_img = self.renderer.create_package(base_sprite, profiles_per_pkg)
            pkg_h, pkg_w = pkg_img.shape[:2]

            pattern = random.choices(["ALTERNATING", "SANDWICH", "UNIFORM"], weights=[0.70, 0.20, 0.10])[0]
            ...
            current_y = self.height - random.randint(20, 80)
            
            render_commands_depth = []
            render_commands_face = []
            ptype_idx = list(self.loader.profiles.keys()).index(specific_ptype)
        ...
        # DURUM 3: STANDART DAĞINIK PROFİLLER (Eski Mantık - Diğerleri)
        else:
            profile_contour = self.loader.profiles[specific_ptype]
            ...
            ref_sprite, _, t = self.renderer.render_profile_sprite(profile_contour, base_size, 0, specific_ptype)
            ph, pw = ref_sprite.shape[:2]
            ...
            while current_y > int(50 * scale_factor):
                ...
                while current_x < self.width - pw:
                    ...
                    sprite, _, cur_t = self.renderer.render_profile_sprite(profile_contour, cur_sz, cur_rot, specific_ptype)
                    sh, sw = sprite.shape[:2]
                    pos_x = current_x; pos_y = row_y + random.randint(-2, 2)
                    if pos_x + sw > self.width: break
                    
                    self._queue_render(render_commands_depth, render_commands_face, sprite, pos_x, pos_y, specific_ptype, ptype_idx, cur_t)
...
def main():
    ...
    print(f"\n🚀 Sentetik Veri V25 (Paket Modeli Entegreli) Başlıyor...")
